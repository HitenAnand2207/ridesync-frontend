...
import { Search, MapPin, Clock, Users, Star, ArrowRight, IndianRupee } from 'lucide-react';
import Link from 'next/link';

export default function SearchGroupsPage() {
  const [form, setForm] = useState({ origin: '', destination: '', departureTime: '', slots: '1' });
  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto' }}>
      ...
      <p style={{ fontSize: '15px', color: 'var(--text-secondary)', marginBottom: '28px' }}>
        Search for batchmates heading your way.
      </p>
        ...
        <form onSubmit={handleSearch}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '20px' }}>
            ...
            <div>
              ...
              <input type="number" min="1" max="4" value={form.slots} onChange={(e) => setForm({ ...form, slots: e.target.value })} style={inputStyle} />
            </div>
          </div>
          ...
          <div key={group.id} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
                ...
                <span style={{ marginLeft: '8px', color: 'var(--text-muted)' }}>· {group._count?.members} member{group._count?.members !== 1 ? 's' : ''}</span>
              </span>
