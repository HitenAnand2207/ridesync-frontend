      ...
      const options = {
        ...
        handler: async (response: any) => {
          try {
            ...
            toast.success('Group is now live! Batchmates can find it.');
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto' }}>
      ...
      <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '28px' }}>
        Create a group and pay ₹30 to make it visible to batchmates.
      </p>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
            <div>
              ...
              <input type="text" value={form.origin} onChange={(e) => setForm({ ...form, origin: e.target.value })} style={inputStyle} placeholder="KIIT University" required />
            </div>
            <div>
              <label style={labelStyle}>To</label>
              <input type="text" value={form.destination} onChange={(e) => setForm({ ...form, destination: e.target.value })} style={inputStyle} placeholder="Bhubaneswar Airport" required />
            </div>
          </div>

          <div style={{ marginBottom: '14px' }}>
            <label style={labelStyle}>Departure date & time</label>
            <input type="datetime-local" value={form.departureTime} onChange={(e) => setForm({ ...form, departureTime: e.target.value })} style={inputStyle} required />
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
            <div>
              <label style={labelStyle}>Total slots (including you)</label>
              <input type="number" min="2" max="6" value={form.totalSlots} onChange={(e) => setForm({ ...form, totalSlots: e.target.value })} style={inputStyle} required />
            </div>
            <div>
              <label style={labelStyle}>Estimated total fare (₹)</label>
              <input type="number" min="0" value={form.estimatedFare} onChange={(e) => setForm({ ...form, estimatedFare: e.target.value })} style={inputStyle} placeholder="400" required />
            </div>
          </div>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
              ...
              placeholder="Early morning flight, AC cab preferred..." />
          </div>
          ...
          <div style={{
            ...
            A one-time <strong style={{ color: 'var(--text-primary)' }}>₹30 platform fee</strong> activates your group and makes it visible to batchmates. This keeps RideSync running.
          </div>
