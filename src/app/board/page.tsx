...
export default function LiveBoardPage() {
  ...
  const handleJoin = async (groupId: string) => {
    ...
    try {
      ...
      toast.success('Joined! Check Telegram for details.');
      fetchGroups(true);
    } catch (err: any) {
  ...
  const filtered = groups.filter(g => {
    const matchesFilter = filter === 'All' || g.destination.toLowerCase().includes(filter.toLowerCase());
    const matchesCity = !userCity || !g.city || g.city.toLowerCase().includes(userCity.toLowerCase());
    return matchesFilter && matchesCity;
  });

  const slotPercent = (g: RideGroup) => ((g.totalSlots - g.availableSlots) / g.totalSlots) * 100;
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '5rem', color: 'var(--text-muted)' }}>Loading board...</div>
      ) : filtered.length === 0 ? (
        <div style={{
          ...
          <p style={{ fontSize: '16px', fontWeight: 500, color: 'var(--text-primary)', marginBottom: '8px' }}>No active groups</p>
          <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginBottom: '20px' }}>Be the first to start a group for this route.</p>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
                      ...
                      <span style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>{group.origin}</span>
                      <ArrowRight size={12} color="var(--text-muted)" />
                      <span style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>{group.destination}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '12px', color: 'var(--text-muted)' }}>
                        <Clock size={11} />{new Date(group.departureTime).toLocaleString()}
                      </span>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
                    ...
                    {group.womenOnly && (
                      <span style={{
                        fontSize: '11px', fontWeight: 600, padding: '4px 10px',
                        borderRadius: '20px', background: '#fce7f3', color: '#be185d',
                      }}>
                        👩 Women only
                      </span>
                    )}
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <span style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <Users size={11} />{group.totalSlots - group.availableSlots}/{group.totalSlots} slots filled
                    </span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    {group.members?.filter(m => m.status === 'CONFIRMED').slice(0, 4).map((m, i) => (
                      <div key={m.id} style={{
                        ...
                        position: 'relative' as const, zIndex: 4 - i,
                      }}>
                        {m.user.name[0]}
                      </div>
                    ))}
                  ...
                  {isMember || isOrganizer ? (
                    ...
                    <button onClick={() => handleJoin(group.id)} disabled={joining === group.id} style={{
                      ...
                      background: group.womenOnly ? '#ec4899' : 'var(--accent)',
